...
Работа с linux началась совсем не так, как хотелось. Вначале я обнаружил, что моя старая виртуалка даже не пытается
запускается.
...
Вначале сгенерируем сертификат. Для этого подготовим [конфиг генерации ключа](nginx/certs/openssl.cnf), передём в
...
## Вывод
- По итогу я не очень удачно, но всё равно полезно повзаимодействовал с виртуальными машинами
- Узнал про основные применения nginx и на практике поразбирался с конфигурацией
- Добавил валидный сертификат и добился признания от браузера
- Получил моральное удовлетворения по завершении работы :)
