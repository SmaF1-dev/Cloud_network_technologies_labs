...
2. Мы не проверяем, готова ли БД 
3. Ну а так же мы не используем ```volumes```, ```healthcheck```, ```.env``` - файл, логирование и разделение структуры
...
Все запускается. Ну правда я сначала полчаса пыталась понять, почему мне вылетает ошибка:
```
docker compose -f good_docker-compose.yml up app

open /Users/polinamitrofanova/Desktop/clouds/Cloud_network_technologies_labs/lab_2/complicated_version/good_docker-compose.yml: no such file or directory
```
Успела уже поссориться с парнем, который рабочий девопс и не хотел мне напрямую подсказывать (good for him, но я до сих пор немного злюсь). Потом только заметила, что спустя полтора месяца забыла свои расширения для компоузов. С парнем помирилась.

Вот скрины с десктопа, что все успешно работает
![image](images/Снимок%20экрана%202025-11-03%20в%2014.48.22.png)

Еще фан факт, спустя почти год вайбкодинга в около-девопс сфере заметила, что оказывается логи в докер десктопе прилетают по Гринвичу. Все это время я думала, что они приходят по utc+3(мск) хихи
![image](images/Снимок%20экрана%202025-11-03%20в%2014.48.37.png)

Это если что хороший компоуз, плохой тоже с горем пополам запустился. 
![image](images/Снимок%20экрана%202025-11-04%20в%2022.05.05.png)
